...
/* Homebound Foundation Consolidation v2 */
const FOUNDATION_BASELINE_VERSION = '2026-08-05-a';
let activeAdventureFilter = 'upcoming';
let fullPageStack = [];

function foundationUpgrade() {
  state.garden = state.garden || clone(defaults.garden);
  state.garden.savingsGoals = Array.isArray(state.garden.savingsGoals) ? state.garden.savingsGoals : [];
  state.notifications = state.notifications && typeof state.notifications === 'object' ? state.notifications : { items: [] };
  state.notifications.items = Array.isArray(state.notifications.items) ? state.notifications.items : [];
  state.adventures = state.adventures && typeof state.adventures === 'object' ? state.adventures : { trips: [] };
  const oldTrips = Array.isArray(state.dreams?.trips) ? state.dreams.trips : [];
  if (!Array.isArray(state.adventures.trips) || !state.adventures.trips.length) state.adventures.trips = oldTrips;
  state.dreams = state.dreams || clone(defaults.dreams);
  state.dreams.items = Array.isArray(state.dreams.items) ? state.dreams.items : clone(defaults.dreams.items);
  state.memories = state.memories && typeof state.memories === 'object' ? state.memories : { items: [] };
  const oldMemories = Array.isArray(state.dreams.memories) ? state.dreams.memories : [];
  if (!Array.isArray(state.memories.items) || !state.memories.items.length) state.memories.items = oldMemories;

  if (state.financialBaselineVersion !== FOUNDATION_BASELINE_VERSION) {
    const balances = { cc1: 8917.13, cc2: 18803.15, 'zip-plus': 3846, 'zip-money': 2509, car: 27617.56 };
    Object.entries(balances).forEach(([id,balance]) => {
      const m = state.milestones.find(x => x.id === id);
      if (m) { m.balance = balance; m.updatedAt = new Date().toISOString(); syncLegacyBalance(m); }
    });
    state.financialBaselineVersion = FOUNDATION_BASELINE_VERSION;
  }

  const dreamDefaults = [
    ['future-home','🏡','Our Future Home','A home and backyard built for our family.','2032'],
    ['family-car','🚙','Next Family Car','A future upgrade when the time is right.','Next year'],
    ['first-dog','🐶','First Dog','A new family member when we are ready.','Someday'],
    ['renovation','🛋️','Backyard Renovation','Create a peaceful space for family life.','Future'],
    ['family-goals','❤️','Family Goals','The bigger plans we want to build together.','Ongoing']
  ];
  dreamDefaults.forEach(([id,icon,name,note,timing]) => {
    let d=state.dreams.items.find(x=>x.id===id);
    if(!d){ d={id,icon,name,note}; state.dreams.items.push(d); }
    d.targetTiming=d.targetTiming||timing; d.preparation=d.preparation||'Not started';
  });
  save();
}

function consumerDebtMetrics() {
  const ids=['cc1','cc2','zip-plus','zip-money'];
  const debts=ids.map(id=>state.milestones.find(m=>m.id===id)).filter(Boolean);
  const original=35985.64;
  const remaining=debts.reduce((s,m)=>s+safeNumber(m.balance),0);
  return {original,remaining,cleared:Math.max(0,original-remaining),percent:original?Math.max(0,(original-remaining)/original*100):0};
}
function carDebtMetrics(){const m=state.milestones.find(x=>x.id==='car');const original=35221;const remaining=safeNumber(m?.balance);return{original,remaining,cleared:Math.max(0,original-remaining),percent:original?Math.max(0,(original-remaining)/original*100):0,m};}
function foundationTotals(){const c=consumerDebtMetrics(),car=carDebtMetrics();return{consumer:c,car,totalRemaining:c.remaining+car.remaining,totalOriginal:c.original+car.original,totalCleared:c.cleared+car.cleared};}

function adventureState(){ state.adventures=state.adventures||{trips:[]}; state.adventures.trips=Array.isArray(state.adventures.trips)?state.adventures.trips:[]; return state.adventures; }
function memoryState(){ state.memories=state.memories||{items:[]}; state.memories.items=Array.isArray(state.memories.items)?state.memories.items:[]; return state.memories; }
function savingsGoals(){state.garden=state.garden||{};state.garden.savingsGoals=Array.isArray(state.garden.savingsGoals)?state.garden.savingsGoals:[];return state.garden.savingsGoals;}

function openFullPage(title, html, options={}) {
  const view=document.getElementById('fullPageView');
  document.getElementById('fullPageToolbarTitle').textContent=title||'';
  document.getElementById('fullPageMore').hidden=options.hideMore!==false;
  document.getElementById('fullPageContent').innerHTML=html;
  view.setAttribute('aria-hidden','false'); document.body.classList.add('full-page-open');
  if(!options.replace) fullPageStack.push({title,html,options});
  view.scrollTop=0;
}
function closeFullPage(){const view=document.getElementById('fullPageView');fullPageStack=[];view.setAttribute('aria-hidden','true');document.body.classList.remove('full-page-open');}

function switchTab(tab) {
  const page=document.getElementById(`${tab}Page`);
  if(!page) return;
  activeTab=tab;
  document.querySelectorAll('.page').forEach(p=>{p.hidden=p!==page;p.classList.toggle('active-page',p===page);});
  document.querySelectorAll('[data-tab]').forEach(button=>{const yes=button.dataset.tab===tab;button.classList.toggle('active',yes);if(yes)button.setAttribute('aria-current','page');else button.removeAttribute('aria-current');});
  if(tab==='garden')renderGarden(); if(tab==='today')renderToday(); if(tab==='dreams')renderDreams(); if(tab==='adventures')renderAdventures(); if(tab==='memories')renderMemories(); if(tab==='journey')renderJourneyFoundation();
  window.scrollTo(0,0);
}

function renderJourneyFoundation(){
  const f=foundationTotals();
  const totalStart=document.getElementById('totalStarting'),totalCleared=document.getElementById('totalCleared'),totalLeft=document.getElementById('totalLeft');
  if(totalStart)totalStart.textContent=money2(f.totalOriginal);if(totalCleared)totalCleared.textContent=money2(f.totalCleared);if(totalLeft)totalLeft.textContent=money2(f.totalRemaining);
  const p=f.totalOriginal?f.totalCleared/f.totalOriginal*100:0;
  document.getElementById('overallJourneyPercent').textContent=`${p.toFixed(1)}%`;
  document.getElementById('ringPercent').textContent=`${p.toFixed(0)}%`;
  document.getElementById('summaryRing').style.setProperty('--ring',`${p}%`);
  const caption=document.getElementById('overallJourneyCaption');if(caption)caption.textContent=`${money2(f.consumer.remaining)} consumer debt • ${money2(f.car.remaining)} car loan`;
  renderMap();
}

function renderDreams(){
  const grid=document.getElementById('dreamGrid'); if(!grid)return;
  grid.innerHTML=state.dreams.items.map(d=>{
    const goal=savingsGoals().find(g=>g.dreamId===d.id);const gp=goal&&safeNumber(goal.target)?Math.min(100,safeNumber(goal.saved)/safeNumber(goal.target)*100):0;
    const holiday=/holiday|trip|vacation|travel/i.test(`${d.name} ${d.note||''}`);
    return `<button class="dream-dashboard-card" data-open-dream="${d.id}"><span class="dream-dashboard-icon">${d.icon||'⭐'}</span><div><strong>${escapeHtml(d.name)}</strong><small>${escapeHtml(d.targetTiming||d.priority||'Future')}</small><p>${escapeHtml(d.note||'A dream worth planning.')}</p>${goal?`<div class="linked-progress"><span>Garden savings</span><b>${gp.toFixed(0)}%</b><i><em style="width:${gp}%"></em></i></div>`:`<span class="dream-status">${escapeHtml(d.preparation||'Not started')}</span>`}</div><span class="arrow">›</span></button>`;
  }).join('')||'<div class="empty-state"><span>⭐</span><strong>No dreams yet</strong><p>Add something you would love to do together.</p></div>';
  const next=state.dreams.items.find(d=>d.targetTiming&&d.targetTiming!=='Someday');const rem=document.getElementById('dreamReminderCard');
  if(rem&&next){rem.hidden=false;rem.innerHTML=`<span>💡</span><div><small>GENTLE REMINDER</small><strong>${escapeHtml(next.name)}</strong><p>Are you ready for the next step, or should we check again later?</p></div><button data-open-dream="${next.id}">Review</button>`;}
}

function openDream(id){
  const d=state.dreams.items.find(x=>x.id===id);if(!d)return;const goal=savingsGoals().find(g=>g.dreamId===id);const holiday=/holiday|trip|vacation|travel/i.test(`${d.name} ${d.note||''}`);
  openFullPage(d.name,`<section class="detail-hero dream-detail-hero"><div class="detail-hero-icon">${d.icon||'⭐'}</div><span>DREAM</span><h1>${escapeHtml(d.name)}</h1><p>${escapeHtml(d.note||'A future worth keeping in focus.')}</p></section><section class="full-section"><div class="detail-row"><span>Target timing</span><strong>${escapeHtml(d.targetTiming||'Not set')}</strong></div><div class="detail-row"><span>Preparation</span><strong>${escapeHtml(d.preparation||'Not started')}</strong></div>${goal?`<div class="goal-linked-card"><small>GARDEN SAVINGS</small><strong>${money2(goal.saved)} of ${money2(goal.target)}</strong><div class="bar large"><i style="--progress:${Math.min(100,safeNumber(goal.saved)/safeNumber(goal.target)*100)}%"></i></div><button class="secondary-button" data-open-goal="${goal.id}">View savings goal</button></div>`:`<button class="primary-button" data-start-saving="${d.id}">🌱 Start Saving</button>`}${holiday?`<button class="secondary-button" data-plan-adventure="${d.id}">🌊 Plan Adventure</button>`:''}</section>`);
}

function renderAdventures(){
  const trips=adventureState().trips;const now=localDateKey();
  const filtered=trips.filter(t=>{if(activeAdventureFilter==='past')return t.archived||t.completedAt||t.endDate&&t.endDate<now;if(activeAdventureFilter==='current')return !t.archived&&t.startDate&&t.endDate&&t.startDate<=now&&t.endDate>=now;return !t.archived&&(!t.startDate||t.startDate>now);});
  const list=document.getElementById('adventureList');if(!list)return;
  list.innerHTML=filtered.map(t=>{const rem=tripRemaining(t),spent=tripSpent(t),p=safeNumber(t.budget)?spent/safeNumber(t.budget)*100:0;return `<button class="adventure-card" data-open-trip="${t.id}"><div class="adventure-cover">${t.coverPhoto?`<img src="${t.coverPhoto}" alt="">`:'🌴'}</div><div><strong>${escapeHtml(t.name)}</strong><small>${escapeHtml(t.destination||'Destination not set')}</small><span>${daysUntil(t.startDate)>0?`${daysUntil(t.startDate)} days to go`:t.archived?'Completed':'Current trip'}</span><div class="linked-progress"><span>${money2(rem)} remaining</span><b>${Math.max(0,100-p).toFixed(0)}%</b><i><em style="width:${Math.max(0,100-p)}%"></em></i></div></div><span class="arrow">›</span></button>`;}).join('')||'<div class="empty-state"><span>🌊</span><strong>No trips here yet</strong><p>Plan an adventure when you are ready.</p></div>';
  const summary=document.getElementById('adventureSummary');if(summary){const upcoming=trips.filter(t=>!t.archived).length;summary.innerHTML=`<div><small>UPCOMING</small><strong>${upcoming}</strong></div><div><small>PLANNED BUDGET</small><strong>${money2(trips.filter(t=>!t.archived).reduce((s,t)=>s+safeNumber(t.budget),0))}</strong></div>`;}
}

function tripFullPage(id, section='overview'){
  const t=adventureState().trips.find(x=>x.id===id);if(!t)return;selectedTripId=id;const rem=tripRemaining(t),spent=tripSpent(t);const tabs=['overview','expenses','itinerary','packing','photos','notes'];
  let body='';
  if(section==='overview')body=`<div class="trip-metrics"><div><small>Total budget</small><strong>${money2(t.budget)}</strong></div><div><small>Spent</small><strong>${money2(spent)}</strong></div><div><small>Remaining</small><strong>${money2(rem)}</strong></div></div><div class="full-menu">${tabs.slice(1).map(s=>`<button data-trip-section="${s}" data-trip-id="${t.id}"><span>${({expenses:'💳',itinerary:'📅',packing:'🧳',photos:'📸',notes:'📝'})[s]}</span><strong>${s[0].toUpperCase()+s.slice(1)}</strong><b>›</b></button>`).join('')}<button data-trip-section="budget" data-trip-id="${t.id}"><span>📊</span><strong>Budget Summary</strong><b>›</b></button></div><button class="primary-button" data-complete-trip="${t.id}">Move to Memories ❤️</button>`;
  if(section==='expenses')body=`<div class="trip-metrics"><div><small>Remaining</small><strong>${money2(rem)}</strong></div><div><small>Total spent</small><strong>${money2(spent)}</strong></div></div><button class="primary-button" data-trip-expense="${t.id}">＋ Add Expense</button><div class="full-list">${(t.expenses||[]).slice().reverse().map(e=>`<article><span>${e.icon||'💳'}</span><div><strong>${escapeHtml(e.category)}</strong><small>${escapeHtml(e.note||e.date||'')}</small></div><b>-${money2(e.amount)}</b></article>`).join('')||'<div class="empty-state"><span>💳</span><strong>No expenses yet</strong><p>Add your first expense to begin tracking the budget.</p></div>'}</div>`;
  if(section==='itinerary')body=`<div class="empty-or-list">${(t.itinerary||[]).map(x=>`<article><span>📅</span><div><strong>${escapeHtml(x.text)}</strong><small>${escapeHtml(x.date||'')}</small></div></article>`).join('')||'<div class="empty-state"><span>📅</span><strong>No itinerary items yet.</strong><p>Start planning your trip by adding your first itinerary item.</p></div>'}</div><button class="primary-button" data-trip-item="itinerary" data-trip-id="${t.id}">＋ Add Itinerary Item</button>`;
  if(section==='packing')body=`<div class="full-list">${(t.packing||[]).map((x,i)=>`<label class="packing-row"><input type="checkbox" data-pack-toggle="${t.id}" data-pack-index="${i}" ${x.done?'checked':''}><span>${escapeHtml(x.text)}</span></label>`).join('')||'<div class="empty-state"><span>🧳</span><strong>Your packing list is empty.</strong><p>Add what you need and tick it off as you pack.</p></div>'}</div><button class="primary-button" data-trip-item="packing" data-trip-id="${t.id}">＋ Add Packing Item</button>`;
  if(section==='photos')body=`<div class="photo-grid">${(t.photos||[]).map(p=>`<button data-cover-photo="${p.id}" data-trip-id="${t.id}"><img src="${p.data}" alt="Trip photo">${t.coverPhoto===p.data?'<span>Cover</span>':''}</button>`).join('')||'<div class="empty-state"><span>📸</span><strong>No photos yet.</strong><p>Choose photos from your gallery when you are ready.</p></div>'}</div><button class="primary-button" data-add-trip-photos="${t.id}">＋ Add Photos</button>`;
  if(section==='notes')body=`<label class="full-notes">Trip notes<textarea id="tripNotesEditor" data-trip-id="${t.id}" placeholder="Write your plans, thoughts or favourite moments…">${escapeHtml(t.notes||'')}</textarea></label><button class="primary-button" data-save-trip-notes="${t.id}">Save Notes</button>`;
  if(section==='budget')body=`<div class="trip-metrics"><div><small>Budget</small><strong>${money2(t.budget)}</strong></div><div><small>Spent</small><strong>${money2(spent)}</strong></div><div><small>Remaining</small><strong>${money2(rem)}</strong></div></div><div class="category-breakdown">${Object.entries((t.expenses||[]).reduce((a,e)=>(a[e.category]=(a[e.category]||0)+safeNumber(e.amount),a),{})).map(([k,v])=>`<div><span>${escapeHtml(k)}</span><strong>${money2(v)}</strong></div>`).join('')||'<p>No expenses recorded yet.</p>'}</div>`;
  openFullPage(t.name,`<section class="detail-hero adventure-detail-hero">${t.coverPhoto?`<img src="${t.coverPhoto}" alt="">`:'<div class="detail-hero-icon">🌴</div>'}<span>ADVENTURE</span><h1>${escapeHtml(t.name)}</h1><p>${escapeHtml(t.startDate||'Dates not set')} ${t.destination?'• '+escapeHtml(t.destination):''}</p></section><nav class="trip-full-tabs">${tabs.map(s=>`<button class="${s===section?'active':''}" data-trip-section="${s}" data-trip-id="${t.id}">${s[0].toUpperCase()+s.slice(1)}</button>`).join('')}</nav><section class="full-section">${body}</section>`,{replace:true});
}

function renderMemories(){
  const items=memoryState().items;const list=document.getElementById('memoryList');if(!list)return;
  list.innerHTML=items.map(m=>`<button class="memory-card" data-open-memory="${m.id}"><div>${m.coverPhoto?`<img src="${m.coverPhoto}" alt="">`:'📖'}</div><section><strong>${escapeHtml(m.name||'Memory')}</strong><small>${escapeHtml(m.startDate||m.completedAt?.slice(0,10)||'')}</small><span>📸 ${(m.photos||[]).length} photos • 📝 ${(m.itinerary||[]).length} moments</span></section><b>›</b></button>`).join('')||'<div class="empty-state"><span>📖</span><strong>Your memories will appear here.</strong><p>Complete an adventure and move it here when you are ready.</p></div>';
  const stats=document.getElementById('memoryStats');if(stats){stats.innerHTML=`<article><small>Adventures</small><strong>${items.length}</strong></article><article><small>Photos</small><strong>${items.reduce((s,m)=>s+(m.photos||[]).length,0)}</strong></article><article><small>Moments</small><strong>${items.reduce((s,m)=>s+(m.itinerary||[]).length,0)}</strong></article>`;}
  const surprise=document.getElementById('memorySurprise');if(surprise&&items.length){const m=items[Math.floor((new Date().getDate()-1)%items.length)];surprise.hidden=false;surprise.innerHTML=`<span>❤️</span><div><small>REMEMBER THIS?</small><strong>${escapeHtml(m.name)}</strong><p>Take a moment to look back at something beautiful.</p></div><button data-open-memory="${m.id}">Open</button>`;}
}
function openMemory(id){const m=memoryState().items.find(x=>x.id===id);if(!m)return;openFullPage(m.name,`<section class="detail-hero memory-detail-hero">${m.coverPhoto?`<img src="${m.coverPhoto}" alt="">`:'<div class="detail-hero-icon">📖</div>'}<span>MEMORY</span><h1>${escapeHtml(m.name)}</h1><p>${escapeHtml(m.startDate||'')} ${m.endDate?'– '+escapeHtml(m.endDate):''}</p></section><section class="full-section"><div class="trip-metrics"><div><small>Total spent</small><strong>${money2(tripSpent(m))}</strong></div><div><small>Budget</small><strong>${money2(m.budget)}</strong></div><div><small>Photos</small><strong>${(m.photos||[]).length}</strong></div></div><h2>Photos</h2><div class="photo-grid">${(m.photos||[]).map(p=>`<button><img src="${p.data}" alt="Memory photo"></button>`).join('')||'<p>No photos were added.</p>'}</div><h2>Story</h2><p class="memory-story">${escapeHtml(m.notes||'A chapter of your family story.')}</p></section>`);}

function createNotification(type,title,message,action='',dedupe=''){
  const items=state.notifications.items;if(dedupe&&items.some(n=>n.dedupe===dedupe&&!n.dismissed))return;
  items.unshift({id:'n-'+Date.now()+'-'+Math.random().toString(36).slice(2),type,title,message,action,dedupe,read:false,dismissed:false,createdAt:new Date().toISOString()});save();
}
function generateNotifications(){
  const trips=adventureState().trips.filter(t=>!t.archived&&t.startDate);trips.forEach(t=>{const days=daysUntil(t.startDate);if(days>=0&&days<=180)createNotification('🌊',t.name,`Your trip is in ${days} days. Check that your budget, bookings and packing are on track.`,`trip:${t.id}`,`trip-${t.id}-${Math.floor(days/14)}`);});
  state.dreams.items.filter(d=>d.targetTiming&&d.targetTiming!=='Someday').slice(0,1).forEach(d=>createNotification('⭐',d.name,'Are you prepared for the next step, or should Homebound remind you later?',`dream:${d.id}`,`dream-${d.id}-${new Date().getMonth()}`));
}
function renderNotifications(){const list=document.getElementById('notificationList'),badge=document.getElementById('notificationBadge');if(!list||!badge)return;const items=state.notifications.items.filter(n=>!n.dismissed);const unread=items.filter(n=>!n.read).length;badge.hidden=!unread;badge.textContent=String(unread);document.getElementById('dismissAllNotifications').hidden=!items.length;list.innerHTML=items.map(n=>`<button data-notification-id="${n.id}" class="notification-item ${n.read?'read':''}"><span>${n.type}</span><div><strong>${escapeHtml(n.title)}</strong><small>${escapeHtml(n.message)}</small></div></button>`).join('')||'<div class="notification-empty"><span>💛</span><strong>You’re all caught up!</strong><p>We’ll let you know when something needs attention.</p></div>';}
function openNotifications(){state.notifications.items.forEach(n=>n.read=true);save();renderNotifications();document.getElementById('notificationsSheet').setAttribute('aria-hidden','false');document.body.classList.add('sheet-open');resetSheetScroll('notificationsSheet');}

function openJourneyChapter(key){
  const ch=JOURNEY_CHAPTERS.find(c=>c.key===key);if(!ch)return;const met=chapterMetrics(ch);
  if(ch.ids.length){const accounts=met.accounts.map(m=>`<button class="account-full-card" data-milestone="${m.id}"><span>${m.icon}</span><div><strong>${escapeHtml(m.accountName)}</strong><small>${money2(m.balance)} remaining</small><i><em style="width:${pct(m)}%"></em></i></div><b>${pct(m).toFixed(0)}%</b></button>`).join('');openFullPage(ch.name,`<section class="detail-hero journey-detail-hero"><div class="detail-hero-icon">${ch.icon}</div><span>JOURNEY CHAPTER</span><h1>${ch.name}</h1><p>${escapeHtml(ch.message)}</p></section><section class="full-section"><div class="trip-metrics"><div><small>Starting</small><strong>${money2(met.starting)}</strong></div><div><small>Cleared</small><strong>${money2(met.starting-met.balance)}</strong></div><div><small>Remaining</small><strong>${money2(met.balance)}</strong></div></div>${accounts}</section>`);return;}
  openFullPage(ch.name,`<section class="detail-hero journey-detail-hero"><div class="detail-hero-icon">${ch.icon}</div><span>FUTURE CHAPTER</span><h1>${ch.name}</h1><p>${escapeHtml(ch.message)}</p></section><section class="full-section"><div class="empty-state"><span>🔒</span><strong>Locked for now</strong><p>Keep progressing through your current chapter to unlock this one.</p></div></section>`);
}

function goalFormForDream(dreamId){const d=state.dreams.items.find(x=>x.id===dreamId);if(!d)return;openFullPage('Start Saving',`<section class="form-page-head"><span>${d.icon||'🌱'}</span><h1>Start Saving for ${escapeHtml(d.name)}</h1><p>Create a goal in Garden.</p></section><section class="full-section"><form id="savingsGoalForm" data-dream-id="${d.id}" class="form-grid"><label>Target amount<input name="target" type="number" min="1" step="0.01" required inputmode="decimal"></label><label>Already saved<input name="saved" type="number" min="0" step="0.01" value="0" inputmode="decimal"></label><label>Target date<input name="targetDate" type="date"></label><label>Why it matters<textarea name="note">${escapeHtml(d.note||'')}</textarea></label><button class="primary-button" type="submit">Create Savings Goal 🌱</button></form></section>`);}
function openGoal(id){const g=savingsGoals().find(x=>x.id===id);if(!g)return;const p=safeNumber(g.target)?Math.min(100,safeNumber(g.saved)/safeNumber(g.target)*100):0;openFullPage(g.name,`<section class="detail-hero garden-detail-hero"><div class="detail-hero-icon">🌱</div><span>SAVINGS GOAL</span><h1>${escapeHtml(g.name)}</h1><p>${escapeHtml(g.note||'Everything you are growing.')}</p></section><section class="full-section"><div class="goal-big-number"><strong>${money2(g.saved)}</strong><small>of ${money2(g.target)}</small><div class="bar large"><i style="--progress:${p}%"></i></div><b>${p.toFixed(0)}%</b></div><div class="goal-actions"><button data-goal-change="deposit" data-goal-id="${g.id}">↓<small>Deposit</small></button><button data-goal-change="withdraw" data-goal-id="${g.id}">↑<small>Withdraw</small></button><button data-goal-history="${g.id}">◷<small>History</small></button></div><div class="full-list">${(g.transactions||[]).slice().reverse().map(x=>`<article><span>${x.type==='deposit'?'🌱':'↗'}</span><div><strong>${x.type==='deposit'?'Deposit':'Withdrawal'}</strong><small>${x.date}</small></div><b>${x.type==='deposit'?'+':'-'}${money2(x.amount)}</b></article>`).join('')||'<p>No transactions yet.</p>'}</div></section>`);}
function goalChangeForm(id,type){const g=savingsGoals().find(x=>x.id===id);if(!g)return;openFullPage(type==='deposit'?'Add to Savings':'Withdraw from Savings',`<section class="form-page-head"><span>${type==='deposit'?'🌱':'↗'}</span><h1>${escapeHtml(g.name)}</h1><p>${money2(g.saved)} currently saved.</p></section><section class="full-section"><form id="goalChangeForm" data-goal-id="${g.id}" data-type="${type}" class="form-grid"><label>Amount<input name="amount" type="number" min="0.01" step="0.01" required inputmode="decimal"></label><label>Date<input name="date" type="date" value="${localDateKey()}"></label><label>Note<input name="note" placeholder="Optional"></label><button class="primary-button" type="submit">Save</button></form></section>`);}

function renderGardenSavingsSection(){const page=document.getElementById('gardenPage');if(!page||document.getElementById('gardenSavingsGoals'))return;const section=document.createElement('section');section.id='gardenSavingsGoals';section.className='garden-section savings-goals-section';section.innerHTML='<div class="section-title-row"><h2>Savings goals 🌱</h2><span class="garden-caption">What you are actively growing</span></div><div id="savingsGoalList" class="savings-goal-list"></div>';const ref=page.querySelector('.garden-section');page.insertBefore(section,ref);}
const originalRenderGarden=renderGarden;
renderGarden=function(){originalRenderGarden();renderGardenSavingsSection();const list=document.getElementById('savingsGoalList');if(list)list.innerHTML=savingsGoals().map(g=>{const p=safeNumber(g.target)?Math.min(100,safeNumber(g.saved)/safeNumber(g.target)*100):0;return `<button class="savings-goal-card" data-open-goal="${g.id}"><span>🌱</span><div><strong>${escapeHtml(g.name)}</strong><small>${money2(g.saved)} of ${money2(g.target)}</small><i><em style="width:${p}%"></em></i></div><b>${p.toFixed(0)}%</b></button>`;}).join('')||'<div class="empty-state compact"><span>🌱</span><strong>No savings goals yet</strong><p>Start one from a Dream.</p></div>';};

function initFoundationUI(){
  foundationUpgrade();generateNotifications();renderNotifications();renderJourneyFoundation();renderDreams();renderGarden();renderToday();renderAdventures();renderMemories();
  const old=document.getElementById('notificationsButton'),cloneBtn=old.cloneNode(true);old.replaceWith(cloneBtn);cloneBtn.addEventListener('click',openNotifications);
  document.getElementById('dismissAllNotifications').addEventListener('click',()=>{state.notifications.items.forEach(n=>n.dismissed=true);save();renderNotifications();});
  document.getElementById('fullPageBack').addEventListener('click',closeFullPage);
  const addAdventure=document.getElementById('adventuresAddButton');if(addAdventure)addAdventure.addEventListener('click',tripForm);
}

document.addEventListener('click',event=>{
  const dream=event.target.closest('[data-open-dream]');if(dream){event.preventDefault();event.stopImmediatePropagation();openDream(dream.dataset.openDream);return;}
  const start=event.target.closest('[data-start-saving]');if(start){event.preventDefault();goalFormForDream(start.dataset.startSaving);return;}
  const plan=event.target.closest('[data-plan-adventure]');if(plan){event.preventDefault();closeFullPage();switchTab('adventures');tripForm();return;}
  const goal=event.target.closest('[data-open-goal]');if(goal){event.preventDefault();openGoal(goal.dataset.openGoal);return;}
  const gc=event.target.closest('[data-goal-change]');if(gc){event.preventDefault();goalChangeForm(gc.dataset.goalId,gc.dataset.goalChange);return;}
  const trip=event.target.closest('[data-open-trip]');if(trip){event.preventDefault();tripFullPage(trip.dataset.openTrip);return;}
  const ts=event.target.closest('[data-trip-section]');if(ts){event.preventDefault();tripFullPage(ts.dataset.tripId,ts.dataset.tripSection);return;}
  const mem=event.target.closest('[data-open-memory]');if(mem){event.preventDefault();openMemory(mem.dataset.openMemory);return;}
  const notes=event.target.closest('[data-save-trip-notes]');if(notes){const t=adventureState().trips.find(x=>x.id===notes.dataset.saveTripNotes);const ta=document.getElementById('tripNotesEditor');if(t&&ta){t.notes=ta.value;save();toast('Notes saved');}return;}
  const af=event.target.closest('[data-adventure-filter]');if(af){activeAdventureFilter=af.dataset.adventureFilter;document.querySelectorAll('[data-adventure-filter]').forEach(b=>b.classList.toggle('active',b===af));renderAdventures();return;}
  const ni=event.target.closest('[data-notification-id]');if(ni){const n=state.notifications.items.find(x=>x.id===ni.dataset.notificationId);if(n){n.read=true;save();closeSheet('notificationsSheet');if(n.action?.startsWith('trip:')){switchTab('adventures');tripFullPage(n.action.split(':')[1]);}else if(n.action?.startsWith('dream:')){switchTab('dreams');openDream(n.action.split(':')[1]);}}return;}
  const gardenCard=event.target.closest('[data-garden-card]');if(gardenCard){event.preventDefault();event.stopImmediatePropagation();const d=gardenDetail(gardenCard.dataset.gardenCard);openFullPage(d[1],`<section class="detail-hero garden-detail-hero"><div class="detail-hero-icon">${d[0]}</div><h1>${d[1]}</h1><p>${d[2]}</p></section><section class="full-section">${d[3]}</section>`);return;}
},true);

document.addEventListener('submit',event=>{
  if(event.target.id==='savingsGoalForm'){event.preventDefault();const f=new FormData(event.target),d=state.dreams.items.find(x=>x.id===event.target.dataset.dreamId);const g={id:'goal-'+Date.now(),dreamId:d.id,name:d.name,target:safeNumber(f.get('target')),saved:safeNumber(f.get('saved')),targetDate:f.get('targetDate')||'',note:f.get('note')||'',transactions:[]};if(g.saved)g.transactions.push({type:'deposit',amount:g.saved,date:localDateKey(),note:'Starting balance'});savingsGoals().push(g);d.linkedGoalId=g.id;save();closeFullPage();switchTab('garden');renderGarden();toast('🌱 Savings goal created');}
  if(event.target.id==='goalChangeForm'){event.preventDefault();const f=new FormData(event.target),g=savingsGoals().find(x=>x.id===event.target.dataset.goalId),amt=safeNumber(f.get('amount')),type=event.target.dataset.type;if(!g||amt<=0)return;g.saved=Math.max(0,safeNumber(g.saved)+(type==='deposit'?amt:-amt));g.transactions=g.transactions||[];g.transactions.push({type,amount:amt,date:f.get('date')||localDateKey(),note:f.get('note')||''});save();openGoal(g.id);renderGarden();toast('Savings updated');}
},true);

// Keep trip creation and storage in Adventures while retaining compatibility.
const oldDreamsState=dreamsState;
dreamsState=function(){const d=oldDreamsState();d.trips=adventureState().trips;d.memories=memoryState().items;return d;};
const oldTripForm=tripForm;
tripForm=function(){oldTripForm();const title=document.getElementById('sheetTitle');if(title)title.textContent='Plan a New Adventure';};
const oldTripSheet=tripSheet;
tripSheet=function(id,section='overview'){tripFullPage(id,section);};
